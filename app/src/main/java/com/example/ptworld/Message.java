...
public class Message extends AppCompatActivity implements View.OnClickListener {
    ...
    String receiveTime;
    String receiveYourNickname;
    Handler handler1;
    String message;

    String yourNickname = FragmentProfile.nickname;
            ...
            public void run() {
                while(true){
                    try {
                        ...
                        receiveNickname = receiveData[1];
                        receiveTime = receiveData[2];
                        try{
                            receiveYourNickname = receiveData[3];
                        }catch(ArrayIndexOutOfBoundsException ai){
                            ai.printStackTrace();
                            receiveYourNickname = TrainnerInfo.nickname;
                        }


                        if(!receiveYourNickname.equals(TrainnerInfo.nickname)){
//                            //닉네임에 알맞은 디바이스토큰을 반환받는다.
                            new Thread_Return_DeviceToken().execute("http://"+IP_ADDRESS+"/user_signup/return_devicetoken.php", receiveYourNickname);
                        }
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                    ...
                    Log.i(TAG,"서버로부터 받은 보낸이 : " + receiveNickname);
                    Log.i(TAG,"서버로부터 받은 시간 : " + receiveTime);
                    Log.i(TAG,"서버로부터 받은 받는이 : " + receiveYourNickname);

                    //내가 말한 메시지이다.
                    if(receiveNickname.equals(TrainnerInfo.nickname)){
                        new Thread(new Runnable() {
                            @Override
                            public void run() {
                                runOnUiThread(new Runnable() {
                                    ...
                                    public void run() {
                                        ...
                                        chattingDTO.nickname = receiveNickname;
    ...
    class Thread_Return_DeviceToken extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            String receiveYourNickname = params[1];

            String postParameters = "nickname=" + receiveYourNickname;
        ...
        protected void onPostExecute(String result) {
            ...
            //알림을 보낸다(채팅 수신자에게)
            new Thread_Push().execute("https://fcm.googleapis.com/fcm/send", device_token, receiveYourNickname);
        }
    }
    private class Thread_Push extends AsyncTask<String, Void, String>  {
        ...
        protected String doInBackground(String... params) {
            ...
            String receiveYourNickname = params[2];
...
//            String postParameters = "nickname=" +  nickname ;
            ...
            data.addProperty("receiveYourNickname",receiveYourNickname);
